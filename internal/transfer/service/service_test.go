...
func TestCreateTransfer_Validation_MissingToWallet(t *testing.T) {
	svc := transferService.NewTransferService(nil, new(MockTransferRepository), new(MockWalletRepository))
...
func TestCreateTransfer_Idempotency_ConflictOnDifferentParams(t *testing.T) {
	tr := new(MockTransferRepository)
	wr := new(MockWalletRepository)
	...
	// Same key, different amount — should be 409.
	_, err := svc.CreateTransfer(context.Background(), &transfer.CreateTransferRequest{
		IdempotencyKey: "key-123",
		FromWalletID:   "wallet_1",
		ToWalletID:     "wallet_2",
...
func TestCreateTransfer_Idempotency_LookupError(t *testing.T) {
	tr := new(MockTransferRepository)
	wr := new(MockWalletRepository)

	tr.On("GetTransferByIdempotencyKey", mock.Anything, mock.Anything, "key-err").
...
func TestGetTransfer_RepoError(t *testing.T) {
	tr := new(MockTransferRepository)
	wr := new(MockWalletRepository)

	tr.On("GetTransferByID", mock.Anything, mock.Anything, "txn-1").
		Return(nil, errors.New("db error"))

	svc := transferService.NewTransferService(nil, tr, wr)
	_, err := svc.GetTransfer(context.Background(), "txn-1")
...
// ─── GetAllTransfers tests ────────────────────────────────────────────────────

func TestGetAllTransfers_Success(t *testing.T) {
	tr := new(MockTransferRepository)
	wr := new(MockWalletRepository)

	transfers := []*transfer.Transfer{
		newProcessedTransfer("txn-1", "wallet_1", "wallet_2", 100),
		newProcessedTransfer("txn-2", "wallet_2", "wallet_3", 50),
	}
	tr.On("GetAllTransfers", mock.Anything, mock.Anything).Return(transfers, nil)

	svc := transferService.NewTransferService(nil, tr, wr)
	result, err := svc.GetAllTransfers(context.Background())

	assert.NoError(t, err)
	assert.Len(t, result, 2)
}

func TestGetAllTransfers_RepoError(t *testing.T) {
	tr := new(MockTransferRepository)
	wr := new(MockWalletRepository)

	tr.On("GetAllTransfers", mock.Anything, mock.Anything).
		Return(nil, errors.New("db error"))

	svc := transferService.NewTransferService(nil, tr, wr)
	_, err := svc.GetAllTransfers(context.Background())
