...
// CreateTransfer executes an atomic wallet-to-wallet transfer.
// Supports idempotency via idempotencyKey and prevents deadlocks by locking wallets in ascending ID order using SELECT … FOR UPDATE.
func (s *transferService) CreateTransfer(ctx context.Context, req *transfer.CreateTransferRequest) (*transfer.TransferResponse, error) {
	...
	if req.FromWalletID == req.ToWalletID {
		logger.Error(fn, static.SameWalletError, nil, map[string]interface{}{"from_wallet": req.FromWalletID, "to_wallet": req.ToWalletID})
	...
	if err != nil {
		logger.Error(fn, "failed to lock first wallet for update", err, map[string]interface{}{"wallet_id": firstID})
		return nil, apperror.InternalServerError("failed to lock wallet")
	}
	if firstWallet == nil {
		if markErr := s.markFailed(ctx, tx, newTransfer.ID, fmt.Sprintf("wallet not found: %s", firstID)); markErr != nil {
			return nil, apperror.InternalServerError("failed to finalize transfer")
		}
		if commitErr := tx.Commit(ctx); commitErr != nil {
			logger.Error(fn, "commit failed after marking transfer failed (wallet not found)", commitErr, map[string]interface{}{
	...
	if err != nil {
		logger.Error(fn, "failed to lock second wallet for update", err, map[string]interface{}{"wallet_id": secondID})
		return nil, apperror.InternalServerError("failed to lock wallet")
	}
	if secondWallet == nil {
		if markErr := s.markFailed(ctx, tx, newTransfer.ID, fmt.Sprintf("wallet not found: %s", secondID)); markErr != nil {
			return nil, apperror.InternalServerError("failed to finalize transfer")
		}
		if commitErr := tx.Commit(ctx); commitErr != nil {
			logger.Error(fn, "commit failed after marking transfer failed (wallet not found)", commitErr, map[string]interface{}{
	...
	if fromWallet.Balance < req.Amount {
		if markErr := s.markFailed(ctx, tx, newTransfer.ID, static.InsufficientBalance); markErr != nil {
			return nil, apperror.InternalServerError("failed to finalize transfer")
		}
		if commitErr := tx.Commit(ctx); commitErr != nil {
			logger.Error(fn, "commit failed after marking transfer failed (insufficient balance)", commitErr, map[string]interface{}{
				"transfer_id": newTransfer.ID,
			})
			return nil, apperror.InternalServerError("failed to finalize transfer")
		}
...
func (s *transferService) GetTransfer(ctx context.Context, id string) (*transfer.TransferResponse, error) {
	...
	if t == nil {
		logger.Info(fn, "transfer not found", map[string]interface{}{"transfer_id": id})
...
func (s *transferService) buildResponse(ctx context.Context, t *transfer.Transfer) (*transfer.TransferResponse, error) {
	...
	if err != nil {
		logger.Error("GetLedgeEntry", "failed to get ledger entries", err, map[string]interface{}{"transfer_id": t.ID})
...
func (s *transferService) markFailed(ctx context.Context, db database.DBTX, id, reason string) error {
	const fn = "markFailed"
