...
func setupIntegration(t *testing.T) (*pgxpool.Pool, func()) {
	...
	teardown := func() {
		_, _ = pool.Exec(context.Background(), `DELETE FROM ledger_entries WHERE transfer_id IN (SELECT id FROM transfers WHERE id LIKE 'test-%')`)
		_, _ = pool.Exec(context.Background(), `DELETE FROM transfers WHERE id LIKE 'test-%' OR idempotency_key LIKE 'integ-%'`)
		_, _ = pool.Exec(context.Background(), `DELETE FROM wallets WHERE id LIKE 'integ-%'`)
	}
